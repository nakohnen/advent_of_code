...
main :: proc() {
    ...
    beams := make([dynamic][dynamic]bool)

    line_nbr := 0
	for line in strings.split_lines_iterator(&it) {
        new_beam_splitters := make([dynamic]bool)
        new_beams := make([dynamic]bool)

        for r, index in line {
            append(&new_beams, false)
            switch r {
            case '.':
        ...
        if line_nbr > 0 {
            for s, index in new_beam_splitters {
                if new_beam_splitters[index] && beams[line_nbr-1][index] {
                    new_beams[index] = false
